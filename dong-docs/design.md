...
1. **브랜드 컬러는 5개뿐.** 새로운 색을 추가하지 않습니다. 음영이 필요하면 기존 5색의 **투명도(opacity)** 로만 파생시킵니다. 라이트/다크는 같은 5토큰의 값만 반전한 것이며, 다크는 1급 모드입니다(대충 반전이 아님).
...
수직 리듬은 `rem` + `clamp()` 기반. 대표 값: masthead 상단 `clamp(3rem, 9vh, 5.5rem)`, 섹션 전환 `2.5–3.5rem`.
...
- **코드:** 인라인은 `--panel` 배경 칩, 블록은 `--panel` + `--line` 테두리 + `10px` radius. 구문 색은 `.c-key`(moss) · `.c-com`(stone italic) · `.c-str`(ink).
- **리스트:** 잎 모양 불릿(위 §3).
