...
export function Masthead() {
  ...
  return (
    <>
      ...
      <header
        className={cn(
          'pt-[clamp(1.15rem,3.5vh,2.75rem)] pb-[0.8rem]',
          'wrap',
        )}
      >
        <Link className="inline-flex items-center gap-[0.6rem] leading-[1.15]" href='/'>
          ...
          <span className="font-medium tracking-[0.02em] text-sm text-stone max-[480px]:hidden">
            — 코드는 쉽지, 말을 보여줘
          </span>
        </Link>
      </header>
      {/* nav — body 직속 sibling이라 페이지 전체에서 상단 고정(sticky) */}
      <nav className="sticky top-0 z-[var(--z-header)] border-b border-line bg-paper" aria-label='주 메뉴'>
        <div
          className={cn(
